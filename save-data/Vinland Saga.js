document.addEventListener('DOMContentLoaded', function() {
  ...
  const averageElement = document.getElementById('averagePage27');

  // Load the saved inputs on page load
  const savedInputs = JSON.parse(localStorage.getItem('savedInputsPage27')) || [];
  ...
  formElement.addEventListener('submit', function(event) {
    ...
    if (inputValue.trim() !== '') {
      ...
      localStorage.setItem('savedInputsPage27', JSON.stringify(savedInputs));
  ...
  const savedReview = localStorage.getItem('savedReviewPage27');
  if (savedReview) {
  ...
  saveReviewButton.addEventListener('click', function(event) {
    ...
    if (review !== '') {
      localStorage.setItem('savedReviewPage27', review);
  ...
  savedReviewsElement.addEventListener('click', function(event) {
    ...
    if (reviewElement.tagName === 'P') {
      ...
      localStorage.removeItem('savedReviewPage27');
    }
  });
});