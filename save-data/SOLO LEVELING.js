document.addEventListener('DOMContentLoaded', function() {
  ...
  const averageElement = document.getElementById('averagePage26');

  // Load the saved inputs on page load
  const savedInputs = JSON.parse(localStorage.getItem('savedInputsPage26')) || [];
  ...
  formElement.addEventListener('submit', function(event) {
    ...
    if (inputValue.trim() !== '') {
      ...
      localStorage.setItem('savedInputsPage26', JSON.stringify(savedInputs));
  ...
  const savedReview = localStorage.getItem('savedReviewPage26');
  if (savedReview) {
  ...
  saveReviewButton.addEventListener('click', function(event) {
    ...
    if (review !== '') {
      localStorage.setItem('savedReviewPage26', review);
  ...
  savedReviewsElement.addEventListener('click', function(event) {
    ...
    if (reviewElement.tagName === 'P') {
      ...
      localStorage.removeItem('savedReviewPage26');
    }
  });
});