document.addEventListener('DOMContentLoaded', function() {
  ...
  const averageElement = document.getElementById('averagePage28');

  // Load the saved inputs on page load
  const savedInputs = JSON.parse(localStorage.getItem('savedInputsPage28')) || [];
  ...
  formElement.addEventListener('submit', function(event) {
    ...
    if (inputValue.trim() !== '') {
      ...
      localStorage.setItem('savedInputsPage28', JSON.stringify(savedInputs));
  ...
  const savedReview = localStorage.getItem('savedReviewPage28');
  if (savedReview) {
  ...
  saveReviewButton.addEventListener('click', function(event) {
    ...
    if (review !== '') {
      localStorage.setItem('savedReviewPage28', review);
  ...
  savedReviewsElement.addEventListener('click', function(event) {
    ...
    if (reviewElement.tagName === 'P') {
      ...
      localStorage.removeItem('savedReviewPage28');
    }
  });
});