document.addEventListener('DOMContentLoaded', function() {
  ...
  const averageElement = document.getElementById('averagePage30');

  // Load the saved inputs on page load
  const savedInputs = JSON.parse(localStorage.getItem('savedInputsPage30')) || [];
  ...
  formElement.addEventListener('submit', function(event) {
    ...
    if (inputValue.trim() !== '') {
      ...
      localStorage.setItem('savedInputsPage30', JSON.stringify(savedInputs));
  ...
  const savedReview = localStorage.getItem('savedReviewPage30');
  if (savedReview) {
  ...
  saveReviewButton.addEventListener('click', function(event) {
    ...
    if (review !== '') {
      localStorage.setItem('savedReviewPage30', review);
  ...
  savedReviewsElement.addEventListener('click', function(event) {
    ...
    if (reviewElement.tagName === 'P') {
      ...
      localStorage.removeItem('savedReviewPage30');
    }
  });
});