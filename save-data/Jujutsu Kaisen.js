document.addEventListener('DOMContentLoaded', function() {
  ...
  const averageElement = document.getElementById('averagePage29');

  // Load the saved inputs on page load
  const savedInputs = JSON.parse(localStorage.getItem('savedInputsPage29')) || [];
  ...
  formElement.addEventListener('submit', function(event) {
    ...
    if (inputValue.trim() !== '') {
      ...
      localStorage.setItem('savedInputsPage29', JSON.stringify(savedInputs));
  ...
  const savedReview = localStorage.getItem('savedReviewPage29');
  if (savedReview) {
  ...
  saveReviewButton.addEventListener('click', function(event) {
    ...
    if (review !== '') {
      localStorage.setItem('savedReviewPage29', review);
  ...
  savedReviewsElement.addEventListener('click', function(event) {
    ...
    if (reviewElement.tagName === 'P') {
      ...
      localStorage.removeItem('savedReviewPage29');
    }
  });
});